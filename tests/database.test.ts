...
describe('database access boundaries (real PostgreSQL engine)', () => {
  ...
  it('returns only the current user’s data', async () => {
    await asUser(alice);
    const result = await db.query<{ user_id: string }>('select user_id from public.mood_entries');
  ...
  it('blocks writing to another owner and mutating mood history', async () => {
    ...
    await expect(
      db.exec(
        `insert into public.mood_entries(id,user_id,score,energy,occurred_at) values(gen_random_uuid(),'${bob}',1,1,now())`,
      ),
    ).rejects.toThrow();
    await expect(db.exec('update public.mood_entries set score=5')).rejects.toThrow();
    await expect(db.exec('delete from public.mood_entries')).rejects.toThrow();
  });
  it('does not expose any health data anonymously', async () => {
    await db.exec('reset role;set role anon');
    await expect(db.query('select * from public.mood_entries')).rejects.toThrow();
  });
  it('prevents self-verification of a trusted contact', async () => {
    await asUser(alice);
    await expect(db.exec('update public.trusted_contacts set verified_at=now()')).rejects.toThrow();
    await expect(db.query('select * from public.contact_tokens')).rejects.toThrow();
    await expect(
      db.query("select public.consume_contact_token('secret','verify')"),
    ).rejects.toThrow();
  });
  it('prevents queueing notifications and reading rate limit internals', async () => {
    await asUser(alice);
    await expect(db.query('select * from public.notification_jobs')).rejects.toThrow();
    await expect(db.query('select * from public.rate_limits')).rejects.toThrow();
    await expect(db.query('select public.claim_notification_jobs(50)')).rejects.toThrow();
  });
  it('prevents cross-owner medication logs even with a valid foreign id', async () => {
    await asUser(alice);
    await expect(
      db.exec(
        `insert into public.routine_logs(id,user_id,routine_id,scheduled_date,scheduled_time,status) values(gen_random_uuid(),'${alice}','30000000-0000-4000-8000-000000000002',current_date,'08:00','taken')`,
      ),
    ).rejects.toThrow();
  });
  it('does not merge two distinct mood observations in one day', async () => {
    await asUser(alice);
    await db.exec(
      `insert into public.mood_entries(id,user_id,score,energy,occurred_at) values(gen_random_uuid(),'${alice}',3,3,now());`,
    );
    const result = await db.query<{ count: number }>(
      'select count(*)::int as count from public.mood_entries',
    );
    expect(result.rows[0].count).toBe(2);
  });
  it('rejects invalid time zones even through the direct database API', async () => {
    await asUser(alice);
    await expect(db.exec("update public.profiles set timezone='Not/AZone'")).rejects.toThrow();
  });
  it('atomically enforces support consent, verified recipient and seven-day cooldown', async () => {
    ...
    const enqueue = async (key: string) => {
      const result = await db.query<{ allowed: boolean }>(
        `select public.enqueue_support_job('${alice}','${recipient}','${key}') as allowed`,
      );
      return result.rows[0].allowed;
    };
    expect(await enqueue('not-consented')).toBe(false);
    await db.exec(
      `update public.profiles set support_enabled=true,consent_at=now() where id='${alice}'`,
    );
    expect(await enqueue('not-verified')).toBe(false);
    await db.exec(`update public.trusted_contacts set verified_at=now() where id='${recipient}'`);
    expect(await enqueue('first')).toBe(true);
    expect(await enqueue('another-scheduler')).toBe(false);
    await db.exec(
      `update public.notification_jobs set status='cancelled';update public.trusted_contacts set enabled=false where id='${recipient}'`,
    );
    expect(await enqueue('revoked')).toBe(false);
    await asUser(alice);
    await expect(
      db.query(`select public.enqueue_support_job('${alice}','${recipient}','unauthorized')`),
    ).rejects.toThrow();
  });
  it('allows explicit owner data deletion without affecting another user', async () => {
    await asUser(alice);
    await db.exec('select public.delete_own_data()');
    const own = await db.query('select * from public.mood_entries');
    expect(own.rows).toHaveLength(0);
    await asUser(bob);
    const other = await db.query('select * from public.mood_entries');
